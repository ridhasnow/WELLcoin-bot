...
const CLAIM_WINDOW = 8*3600*1000;

// State
let daily = null, timerId = null;

// Improved state machine: roll forward instead of hard reset when user misses windows
function ensureStateInit(u, tNow){
  ...
  if (!u.meDailyState){
    ...
    if (!st.nextAvailableAt || !st.expiresAt){
      ...
      st.expiresAt = tNow + CLAIM_WINDOW;
    }
    // Soft roll-forward: for every missed 24h window, advance the day instead of resetting to day 1
    while (tNow > (st.expiresAt||0) && tNow >= (st.nextAvailableAt||0)){
      ...
      st.nextAvailableAt = Number(st.nextAvailableAt||tNow) + ONE_DAY;
      st.expiresAt = st.nextAvailableAt + CLAIM_WINDOW;
    }
    u.meDailyState = st;
  }
...
function showPop(title, desc, reward){
  ...
  const rEl = document.getElementById('pop-reward');
  rEl.style.display = reward ? '' : 'none';
  rEl.textContent = reward || '';
  mask.style.display='flex';
}
popOk.addEventListener('click', ()=>{ mask.style.display='none'; });

// Withdraw custom popup
...
async function claim(userRef, day){
  ...
  await userRef.transaction(u=>{
    ...
    if (day >= 30){
      st.currentDay = 1;
      st.nextAvailableAt = tNow + ONE_DAY;
      st.expiresAt = st.nextAvailableAt + CLAIM_WINDOW;
      st.claimedDays = {};
    } else {
      st.currentDay = day + 1;
  ...
  if (reward){
    ...
    (function fire(){ const c = document.getElementById('confetti'); if (c) confetti(); })();
  }
}
